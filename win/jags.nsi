!define APP_NAME "JAGS-alcove"
!define PUBLISHER "JAGS-alcove"

;Name used for JAGS Wiener module registry keys
!define ALCOVE_KEYNAME "${APP_NAME}-${VERSION}"
;Name visible to users, for shortcuts, installation directories, etc.
!define ALCOVE_VISIBLE_NAME  "${APP_NAME} ${VERSION}"
...
!define INSTDIR_REG_KEY "Software\Microsoft\Windows\CurrentVersion\Uninstall\${ALCOVE_KEYNAME}"
...
!define UNINSTALL_LOG "Uninstall-alcove"
...
Name "${ALCOVE_VISIBLE_NAME}"
OutFile "${APP_NAME}-${VERSION}.exe"

!define APP_REG_KEY "Software\${PUBLISHER}\${ALCOVE_KEYNAME}"
...
Section #Default section
   ...
   WriteRegStr ${INSTDIR_REG_ROOT} "${INSTDIR_REG_KEY}" "DisplayName" "${ALCOVE_VISIBLE_NAME}"
   ...
   WriteRegStr ${INSTDIR_REG_ROOT} "${INSTDIR_REG_KEY}" "URLInfoAbout" "http://jags-alcove.sourceforge.net"
...
Section "32-bit installation" Sec32
   ...
   File /r inst32\lib\JAGS\modules-${MAJOR}\alcove*
...
Section "64-bit installation" Sec64
   ...
   !insertmacro UNINSTALL.LOG_OPEN_INSTALL
   File /r inst64\lib\JAGS\modules-${MAJOR}\alcove*
   !insertmacro UNINSTALL.LOG_CLOSE_INSTALL
